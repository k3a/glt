...
#include <string>
#include <string.h>
#include "../include/glv_textview.h"
...
bool TextView::onEvent(Event::t e, GLV& g){
	...
	switch(e){
		case Event::KeyDown:
			if(isprint(key)){
				deleteSelected();
                if (mText.size()<mPos) mPos = (int)mText.size(); // k3a: fix when typing after setting a short text
				...
				case Key::Down:	setPos((int)mText.size()); return false;
...
int TextView::xToPos(float x){
	...
	if(p > (int)mText.size()) p = (int)mText.size();
	if(p<0) p=0;
	return p;
}
