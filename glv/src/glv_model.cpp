...
template<> int fromToken<std::string>(std::string& dst, const std::string& src){
	int b = (int)src.find_first_not_of("\"");
	int e = (int)src.find_last_not_of("\"");
...
int ModelManager::snapshotsToFile(const std::string& path) const {
	...
	if(fp){
		r = (int)fwrite(s.c_str(), sizeof(std::string::value_type), s.size(), fp);
		fclose(fp);
	}
	return r;
}
...
int ModelManager::snapshotsFromFile(const std::string& path, bool add){
	...
	if(fp){
		...
		std::string s;
		while(!feof(fp)){
			int n = (int)fread(buf, sizeof(buf[0]), sizeof(buf), fp);
			s.append(buf, n);
			r += n;
		}
...
struct KeyValueParser{
	...
	int operator()(const std::string& src){
		...
		unsigned p = (unsigned)src.find_first_of("{");
		if(std::string::npos == (int)p) return 0;		// no table found, so return
		...
		if(*b == '}')	return (int) (b+1-&src[0]);
		else			return (int) (b-&src[0]);
	}
};
...
static bool goToNext(unsigned& p, char c, const std::string& str){
	unsigned n = (unsigned)str.substr(p).find_first_of(c);
//	printf("%c: %d %d (%.5s ...)\n", c, p, n, str.substr(p).c_str());
	if(std::string::npos != (int)n){
		p += n;
		return true;
	}
	return false;
...
static bool goToNextPrintablePast(unsigned& p, char c, const std::string& str){
	unsigned n = (unsigned)str.substr(p).find_first_not_of(" \t\r\n");
//	printf("%c: %d %d (%.5s ...)\n", c, p, n, str.substr(p).c_str());
	if(std::string::npos != (int)n){
		p += n;
//		printf("\t%c\n", str[p]);
